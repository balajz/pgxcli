...
func GetTableStyle(tc *config.TableConfig) renderer.ColorizedConfig {
	...
	colorCfg.Header = renderer.Tint{FG: renderer.Colors{getHeaderColor(tc.Color.Header)}}
	colorCfg.Column = renderer.Tint{FG: renderer.Colors{getColumnColor(tc.Color.Column)}}
	...
	colorCfg.Symbols = tw.NewSymbols(resolveStyle(tc.Style))
	return colorCfg
}
